...
const Footer = () => {
  ...
  const socialLinks = [
    ...
    {
      icon: Linkedin,
      href: "https://linkedin.com/in/ayushdama",
      label: "LinkedIn",
  ...
  return (
    <footer
      ref={footerRef}
      className={`relative overflow-hidden border-t ${isDark ? "bg-slate-900 border-slate-800" : "bg-slate-50 border-slate-200"}`}
    >
        ...
        >
          ...
          <motion.div variants={itemVariants} className="col-span-1 md:col-span-2">
            ...
            <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed max-w-md">
              Computer Science student at Lovely Professional University and passionate about building responsive, functional web and ML-based applications that make real impact.
            </p>
            ...
            <div className="mt-6 flex items-center gap-4">
              {socialLinks.map(({ icon: Icon, href, label, hoverColor }) => (
                <motion.a
                  ...
                  whileTap={{ scale: 0.95 }}
                  className={`w-10 h-10 flex items-center justify-center rounded-lg ${isDark ? "bg-slate-800 text-slate-300" : "bg-white text-slate-700 shadow-sm"} ${hoverColor} hover:text-white transition-colors`}
                >
                  <Icon size={18} />
                </motion.a>
              ))}
            ...
            <ul className="space-y-3">
              {[
                ...
                <li key={item.name}>
                  <a
                    href={item.href}
                    className={`text-slate-600 dark:text-slate-400 hover:text-primary-500 dark:hover:text-primary-400 transition-colors text-sm flex items-center gap-1 group`}
                  >
                    <span
                      className={`inline-block w-1.5 h-1.5 rounded-full ${isDark ? "bg-slate-700" : "bg-slate-300"} group-hover:bg-primary-500 dark:group-hover:bg-primary-400 transition-colors`}
                    ></span>
                    {item.name}
                  </a>
                </li>
            ...
            <ul className="space-y-3">
              ...
              <li>
                ...
                >
                  ...
                  <span className="break-all">ayushdama07@gmail.com</span>
                </a>
              </li>
              <li>
                <a
